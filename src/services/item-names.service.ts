...
export class ItemNamesService {
  ...
  convertItemName(itemId, eventType, items) {
  ...
  static isItem(itemId: number): boolean {
    return itemId >= 100;
  }

  static isDungeonItem(itemId: number): boolean {
    return itemId >= 200 && itemId <= 250;
  }

  static isProgressiveItem(itemId: number): boolean {
    return itemId >= 150 && itemId < 200;
  }

  static isTrackableItem(itemId: number): boolean {
    return itemId >= 100;
  }
}
